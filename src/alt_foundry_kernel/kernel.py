...
def run_kernel_transition(
    ...
    if typed_packet.type == PacketType.ADMISSION:
        ...
        entry = _entry(
            ...
            "Admission packet satisfied v0.4.0 settlement gates.",
            capital_delta=capital_delta,
    ...
    if typed_packet.type == PacketType.BRIDGE:
        entry = _entry(
            ...
            Decision.DEFER,
            typed_packet.state,
            "Bridge packet recorded; v0.4.0 does not apply bridge semantics.",
        )
        _append_audit(next_state, entry)
        return TransitionResult(
            decision=Decision.DEFER,
    ...
    return _reject_result(
        ...
        Decision.DEFER,
        typed_packet.state,
        "Packet type is parsed but v0.4.0 does not perform a capital-changing transition.",
        signed,
    )
